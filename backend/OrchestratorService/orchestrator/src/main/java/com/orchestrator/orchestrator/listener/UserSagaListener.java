...
public class UserSagaListener {
    ...
    public void onClientCriado(@Payload SagaMessage<ClientDTO> message) {
        try {
            ...
            System.out.println(
                    "[ORQUESTRADOR] Serviço " + origin + " confirmou sucesso (correlationId: " + correlationId + ")");
            ...
            if (sagaStateManager.isComplete(correlationId)) {
                if (sagaStateManager.get(correlationId).hasFailure()) {
                    System.out.println(
                            "[ORQUESTRADOR] SAGA CLIENTE CONCLUÍDA COM ERRO (correlationId: " + correlationId + ")");
                    sagaUserService.compensateSuccessfulServicesClient(correlationId, message.getPayload());
                } else {
                    System.out.println(
                            "[ORQUESTRADOR] SAGA CLIENTE CONCLUÍDA COM SUCESSO (correlationId: " + correlationId + ")");
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void onFuncionarioCriado(@Payload SagaMessage<EmployeeDTO> message) {
        try {
            ...
            if (sagaStateManager.isComplete(correlationId)) {
                if (sagaStateManager.get(correlationId).hasFailure()) {
                    System.out.println("[ORQUESTRADOR] SAGA FUNCIONÁRIO CONCLUÍDA COM ERRO (correlationId: "
                            + correlationId + ")");
                    sagaUserService.compensateSuccessfulServicesEmployee(correlationId, message.getPayload());
                } else {
                    System.out.println("[ORQUESTRADOR] SAGA FUNCIONÁRIO CONCLUÍDA COM SUCESSO (correlationId: "
                            + correlationId + ")");
                }
            }
        } catch (Exception e) {
    ...
    public void onFuncionarioErro(@Payload SagaMessage<EmployeeDTO> message) {
        try {
            ...
            if (sagaStateManager.isComplete(correlationId)) {
                System.out.println(
                        "[ORQUESTRADOR] SAGA FUNCIONÁRIO CONCLUÍDA COM ERRO (correlationId: " + correlationId + ")");
                sagaUserService.compensateSuccessfulServicesEmployee(correlationId, message.getPayload());
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void onFuncionarioExclusaoSucesso(@Payload SagaMessage<EmployeeDTO> message) {
        try {
            ...
            if (sagaStateManager.isComplete(correlationId)) {
                if (sagaStateManager.get(correlationId).hasFailure()) {
                    System.out.println("[ORQUESTRADOR] SAGA DE EXCLUSÃO FUNCIONÁRIO CONCLUÍDA COM ERRO (correlationId: "
                            + correlationId + ")");
                    sagaUserService.compensateSuccessfulServicesEmployee(correlationId, message.getPayload());
                } else {
                    System.out.println(
                            "[ORQUESTRADOR] SAGA DE EXCLUSÃO FUNCIONÁRIO CONCLUÍDA COM SUCESSO (correlationId: "
                                    + correlationId + ")");
                }
            }
        } catch (Exception e) {
    ...
    @RabbitListener(queues = "funcionario.editar.sucesso")
    public void onFuncionarioEdicaoSucesso(@Payload SagaMessage<EmployeeDTO> message) {
        try {
            String correlationId = message.getCorrelationId();
            String origin = message.getOrigin();

            System.out.println("[ORQUESTRADOR] Serviço " + origin + " confirmou edição com sucesso (correlationId: "
                    + correlationId + ")");
            ...
            if (sagaStateManager.isComplete(correlationId)) {
                if (sagaStateManager.get(correlationId).hasFailure()) {
                    System.out.println("[ORQUESTRADOR] SAGA DE EDIÇÃO FUNCIONÁRIO CONCLUÍDA COM ERRO (correlationId: "
                            + correlationId + ")");
                    // sagaUserService.compensateSuccessfulServicesEmployee(correlationId,
                    // message.getPayload());
                } else {
                    System.out
                            .println("[ORQUESTRADOR] SAGA DE EDIÇÃO FUNCIONÁRIO CONCLUÍDA COM SUCESSO (correlationId: "
                                    + correlationId + ")");
                }
            }
        } catch (Exception e) {
    ...
    @RabbitListener(queues = "funcionario.editar.falhou")
    public void onFuncionarioEdicaoFalhou(@Payload SagaMessage<EmployeeDTO> message) {
        try {
            String correlationId = message.getCorrelationId();
            String origin = message.getOrigin();

            System.out.println(
                    "[ORQUESTRADOR] Serviço " + origin + " FALHOU na edição (correlationId: " + correlationId + ")");
